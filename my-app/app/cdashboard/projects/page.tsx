...
const assignProject = async (project) => {
  try {
    ...
    // Map to store the maximum score and userId for each skill
    const maxScores = {};

    // Find the maximum score and userId for each required skill
    testResultsSnapshot.forEach((doc) => {
      ...
      requiredSkills.forEach((skill) => {
        ...
        if (!maxScores[skill] || score > maxScores[skill].score) {
          maxScores[skill] = { userId, score };
        }
      });
    });

    // Render the project card with the userIds for each required skill
    return (
          ...
          <div className="flex flex-wrap gap-1 font-mono">
            {project.skills.map((skill) => (
              <span key={skill} className="rounded-full bg-gray-200 px-2 py-1 text-xs">
                {skill}
                {maxScores[skill] && ` (Max Score: ${maxScores[skill].userId})`}
              </span>
            ))}
          </div>
        </div>
